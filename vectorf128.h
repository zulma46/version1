...
static inline __m128 selectf (__m128 const & s, __m128 const & a, __m128 const & b) {
    ...
    // to do in version 2: _mm_blend_ps if s is a compile time constant
#else
    return _mm_or_ps(
        _mm_and_ps(s,a),
        _mm_andnot_ps(s,b));
...
static inline Vec4f maximum(Vec4f const & a, Vec4f const & b) {
    return select(a != a, a, max(a,b)); 
}

// same as min, NAN-safe
static inline Vec4f minimum(Vec4f const & a, Vec4f const & b) {
...
#elif INSTRSET >= 10 && defined(_mm_rsqrt14_ps) // missing in VS2019
...
* Vec4f c;
* c = blend4<1,4,-1,7> (a,b);         // c is (11, 20,  0, 23)
*
...
#if  INSTRSET >= 10  // AVX512VL
    __mmask8 mmz = (i0>=0) | (i1>=0)<<1 | (i2>=0)<<2 | (i3>=0)<<3; 
    return _mm_maskz_shuffle_ps (mmz, a, a, (i0&3) | (i1&3)<<2 | (i2&3)<<4 | (i3&3)<<6);
...
// same as max, NAN-safe
static inline Vec2d maximum(Vec2d const & a, Vec2d const & b) {
    return select(a != a, a, max(a,b)); 
}

// same as min, NAN-safe
static inline Vec2d minimum(Vec2d const & a, Vec2d const & b) {
    return select(a != a, a, min(a,b)); 
} 
...
* Vec2d b, c;
* b = permute2<1,1>(a);          // b is (11, 11)
* c = permute2<-1,0>(a);         // c is ( 0, 10)
*
*
* The blend function can mix elements from two different vectors and
* optionally set some elements to zero. 
...
#define permute2d  permute2
#define permute4f  permute4
#define blend2d    blend2
#define blend4f    blend4
...
#elif INSTRSET >= 9  //  __AVX512F__
    __m512i indx = _mm512_castsi128_si512(constant4i<i0,i1,i2,i3>());
